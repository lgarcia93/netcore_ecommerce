...
resource "aws_apigatewayv2_integration" "apigateway_private_integration_cart" {
  api_id           = aws_apigatewayv2_api.ecommerceapi.id
  integration_type = "HTTP_PROXY"
  integration_uri  = aws_lb_listener.nlb-listener-cart-service.arn
...
resource "aws_apigatewayv2_integration" "apigateway_private_integration_orders" {
  api_id           = aws_apigatewayv2_api.ecommerceapi.id
  integration_type = "HTTP_PROXY"
  integration_uri  = aws_lb_listener.nlb-listener-order-service.arn
