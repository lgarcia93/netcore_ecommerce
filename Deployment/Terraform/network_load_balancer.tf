...
#Creating load balancer listener for cart service
resource "aws_lb_listener" "nlb-listener-cart-service" {
  load_balancer_arn = aws_lb.network_ecommerce_loadbalancer.arn
  protocol          = "TCP"
  port              = 5001
...
resource "aws_lb_target_group" "nlb-tg-5001-nlb" {
  name        = "nlb-tg-5001-alb"
  port        = 5001
  protocol    = "TCP"
  vpc_id      = aws_vpc.ecommerce_vpc.id
  target_type = "alb"
...
##Creating load balancer target group attachment (forwarding to ALB)
resource "aws_lb_target_group_attachment" "g-attachment-alb-5001" {
  target_group_arn = aws_lb_target_group.nlb-tg-5001-nlb.arn
  target_id        = aws_lb.ecom_alb.arn
  port             = 5001
}